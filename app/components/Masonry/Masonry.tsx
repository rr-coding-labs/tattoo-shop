...
const Masonry: React.FC<MasonryProps> = ({
  ...
  useEffect(() => {
    if (totalHeight > 0) onHeightChange?.(totalHeight);
    ...
    <div ref={containerRef} className="list">
      {grid.map((item, index) => (
        <div
          ...
          onClick={() => onItemClick ? onItemClick(index) : window.open(item.url, '_blank', 'noopener')}
          onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onItemClick ? onItemClick(index) : window.open(item.url, '_blank', 'noopener'); } }}
        ...
        >
          <div className="item-img">
            <img
              src={item.img}
              alt={`Tattoo portfolio piece ${index + 1}`}
              style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center', display: 'block', borderRadius: '10px' }}
            />
            {colorShiftOnHover && (
              <div className="color-overlay" aria-hidden="true" style={{
                position: 'absolute', top: 0, left: 0,
                width: '100%', height: '100%',
                background: 'linear-gradient(45deg, rgba(184,115,51,0.5), rgba(212,148,74,0.5))',
